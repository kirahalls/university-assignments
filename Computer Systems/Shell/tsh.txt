...
void eval(char *cmdline) 
{
  ...
  //Need to execute piped commands since command 2 isn't null
  if(cmd2 != NULL)
  {
    ...
    if (!executed)
    {
      ...
      else 
      {
        ...
        else if (bg)
        {
          ...
          //Output information about background job
          sio_puts("[");
          sio_putl(job_id);
          sio_puts("] (");
          sio_putl(pid);
...
int builtin_cmd(char **argv) 
{
  ...
  //Just want to exit the shell if "quit"
  if (! strcmp(cmd, "quit")) 
  {
    exit(0);
    return 1;
  ...
  if (! strcmp(cmd, "jobs"))
  {
    for (int i = 0; i < MAXJOBS; i++) {
      if (jobs[i].pid != 0) {
        ...
        if (jobs[i].state == BG)
        {
          //Output information about the job that is running
          sio_puts("[");
          sio_putl(jobs[i].jid);
          sio_puts("] (");
          sio_putl(jobs[i].pid);
        ...
        {
          //Output information about the stopped job
          sio_puts("[");
          sio_putl(jobs[i].jid);
          sio_puts("] (");
          sio_putl(jobs[i].pid);
...
void do_bg(int jid) 
{
  ...
  if (current_job == NULL)
  {
    //No background job with this jid
    sio_puts("%");
    sio_putl(jid);
  ...
  //Output information about the job that was switched
  sio_puts("[");
  sio_putl(jid);
  sio_puts("] (");
...
void do_fg(int jid) 
{
  ...
  //Send SIGCONT command to the job
  current_job->state = FG;
  kill(current_job->pid * -1, SIGCONT);
...
void waitfg(pid_t pid)
{
  ...
  //While there is a job in the foreground, send sigsuspend
...
void sigchld_handler(int sig) 
{
  ...
  {
    //If receive a stopped signal, change the job state to stopped and output stopped info
    ...
    //If receive another signal, output information about the job and delete it from the job list
    ...
    //If not either of those two signals, just delete the job
    else 
    {
      deletejob(jobs, pid);
    }
...
void sigint_handler(int sig) 
{
  ...
  for (int i = 0; i < MAXJOBS; i++) 
  {
    //Send the sigint signal to every process in the foreground group
    if (jobs[i].state == FG) 
    {
...
void sigtstp_handler(int sig) 
{
  ...
  for (int i = 0; i < MAXJOBS; i++) 
  {
    //Send the SIGTSTP signal to every process in the foreground group
    if (jobs[i].state == FG) 
    {
