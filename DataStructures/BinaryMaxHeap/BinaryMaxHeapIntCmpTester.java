...
/**
 * This class contains JUnit tests for an Integer type of BinaryMaxHeap.java with a custom comparator.
 *
 * @author Nandini Goel and Kira Halls
 * @version Nov 27, 2022
 */
class BinaryMaxHeapIntCmpTester {
    ...
    private BinaryMaxHeap<Integer> intMaxHeap = new BinaryMaxHeap<>(intCmp);
    private BinaryMaxHeap<Integer> emptyMaxHeap = new BinaryMaxHeap<>(intCmp);
    ...
    @BeforeEach
    void setUp() {
        emptyMaxHeapList = new BinaryMaxHeap<>(list, intCmp);
        for(int i = 0; i <= 9; i++){
            intMaxHeap.add(i + 1);
            list.add(i+1);
        }
        intMaxHeapList = new BinaryMaxHeap<>(list, intCmp);
    }
