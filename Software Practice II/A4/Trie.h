...
private: 
    ...
    std::vector<std::string> prefixHelper(std::string prefix);
...
public:
    ...
    /// @brief The destructor for a trie object
    ~Trie();

    /// @brief The copy constructor to create a trie that is an independent copy of another
    /// @param that The trie to copy 
    Trie(const Trie& that);

    /// @brief The copy assignment operator to assign a trie object as an independent copy of another trie
    /// @param that The trie to be copied
    /// @return A reference to the copied trie
    Trie& operator=(Trie that);
